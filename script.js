...
function loadQuestion(index) {
    ...
    prevBtn.disabled = index === 0 || index === 12;
...
function startTimer() {
    ...
    timerInterval = setInterval(() => {
        ...
        updateTimerDisplay();
        if (timeLeft <= 0) {
            if (currentSection === 1) proceedToSection2();
            else finishTest();
        }
    }, 1000);
}
...
function updateTimerDisplay() {
    ...
    timerDisplay.style.color = timeLeft < 60 ? '#ff4444' : 'white';
}
...
function calculateResults() {
    ...
    questions.forEach((q, i) => {
        ...
        if (q.section === "Reading & Writing" && isCorrect) rwScore++;
        if (q.section === "Math" && isCorrect) mathScore++;
    ...
    for (const [skill, data] of Object.entries(skillScores)) {
        ...
        if (percentage >= 85) { performance = "Strong"; dotClass = "strong"; }
        else if (percentage >= 70) { performance = "Developing"; dotClass = "developing"; }
        else { performance = "Needs Improvement"; dotClass = "needs-improvement"; }
